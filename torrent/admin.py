...
from django.contrib import admin
from .models import Category, Tracker, Project, Torrent, TrackerStat, ExternalTorrent
...
class CategoryAdmin(admin.ModelAdmin):
    ...
    def parent_category_name(self, obj):
        return obj.category_parent_id.name if obj.category_parent_id else None
...
class ProjectAdmin(admin.ModelAdmin):
    list_display = ('name', 'user', 'is_active', 'creation', 'deletion')
    prepopulated_fields = {"slug": ("name",)}

@admin.register(Torrent)
class TorrentAdmin(admin.ModelAdmin):
    list_display = ('name', 'info_hash', 'category', 'project', 'is_active', 'uploader', 'creation', 'deletion')
    prepopulated_fields = {"slug": ("name",)}
    search_fields = ('name', 'info_hash')

@admin.register(TrackerStat)
class TrackerStatAdmin(admin.ModelAdmin):
    list_display = ('torrent', 'tracker', 'level', 'seed', 'leech', 'complete')

@admin.register(ExternalTorrent)
class ExternalTorrentAdmin(admin.ModelAdmin):
    list_display = ('url',)
